...
- 🤝 **Every page is uncacheable — decide whether to make public pages edge-cacheable.** (From the SEO audit, 2026-08-07.) Every SSR route returns `cache-control: private, no-cache, no-store, max-age=0, must-revalidate`, which is Next's default when `getServerSideProps` does not set one. Nothing caches at Vercel's edge, so all 16,087 poem pages cost a full render plus a backend round-trip on **every** Googlebot fetch — and crawl rate is throttled by how expensive a site is to crawl. Measured TTFB is fine (0.3-0.55s); the issue is that it is paid every single time.

  **This is a product decision, not a header tweak, which is why it is 🤝.** These pages embed `initialUser`, so the HTML genuinely differs per visitor and cannot be blanket-cached. Making them anonymous-cacheable means dropping `initialUser` from SSR and hydrating auth client-side — `/api/auth/session` already exists for exactly that — and accepting a brief signed-out header on first paint for logged-in readers. Weigh that against roughly 16k pages becoming free to serve. Do NOT just add `s-maxage` without moving `initialUser`: that would serve one visitor's signed-in header to everyone.

  Related and much smaller: the sitemap routes DO set `public, s-maxage=86400, stale-while-revalidate` and only `public` survives to the CDN (`x-vercel-cache: MISS`, `age: 0`), so `poems-famous.xml` fully regenerates on every crawl. Likely the bare `stale-while-revalidate` with no value being dropped in normalization — give it a value and re-check the response headers.

- 🤖 **Author-page pagination.** (From the SEO audit, 2026-08-07.) `/authors/<slug>` renders its first 10 poems and stops; `?page=` is not honoured there, unlike `/[genre]` and `/`. **408 authors have more than 10 poems, and 3,381 poems sit past page 1 of their author page** — 21% of the collection, largest author 85 poems. Those poems ARE reachable via genre pagination now, so this is a second and more topical path rather than the only one, which is why it is no longer urgent. (An earlier note called this low-impact by reasoning from the ~5-poem average; that was wrong, hence the counts.)

  Cheap now, because the parts exist: `components/Pagination.tsx`, `src/utils/pagination.ts` (`parsePageParam`, `buildPageHref`, `pageCount`), the 404-past-the-end rule and the self-canonical logic all shipped with the list pagination. It is wiring `useAuthorPoems` and `pages/authors/[slug].tsx` to them, plus `usePageUrlSync` if the scroll-following URL is wanted there too. Follow the same rules — page 1 is the clean URL, `?page=1` and junk redirect, past-the-end is 404, each page self-canonical, title carries the page number.

- 🤖 **SEO smalls — homepage structured data, the homepage `h1`, and two missing canonicals.** (From the SEO audit, 2026-08-07.) Individually minor, one contained commit together:
  - **No JSON-LD on `/` or `/authors` at all.** The homepage is the strongest page on the site and emits none. Add `WebSite` — with `SearchAction`, which is what can earn a sitelinks searchbox — and `Organization`. Builders already exist in `src/utils/structuredData.ts` and `components/JsonLd.tsx` serialises them.
  - **The homepage `<h1>` is "Poems".** It says nothing; the `<title>` ("Your poem community") is far stronger. Note it is `sr-only` on the dashboard by deliberate design — see the comment in `Dashboard.tsx` — so this is a wording change, not a visibility one.
  - **`/privacy` and `/terms` carry no `<link rel="canonical">`.** Both are indexable and in the sitemap, so they are the only indexable pages on the site without one.
  - **No `og:image:width` / `og:image:height`.** The image itself is a correct 1200x630; the tags just let scrapers lay the card out before fetching it.
  - **`/login` and `/register` have no `<h1>`.** Accessibility rather than SEO — both are `noindex,nofollow` — but it is the same file to touch.
