...
- 👤 **Verify author pagination on the live site** (2026-08-10, follows the change below). Not inferable from CI: check `/authors/<a prolific poet>?page=2` renders the second ten poems, that `?page=1` 307s to the clean URL, that `?page=9999` is a real 404, and that clicking through the nav from inside the site swaps the poems rather than appending to them. `mordecai-ben-isaac` and the 85-poem author are the useful cases.
