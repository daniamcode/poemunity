...
- **Nine commits are unverified in production.** Six from 2026-08-10 — frontend: `978fa24` author pagination, `dc341f7` SEO smalls, `be58670` sitemap cache header; backend: `5aeb0dd` legacy `/users` routes deleted, `59b3cf9` `?genre=` escaped, `c42e4ca` 400s + the empty-poem guard. **The three backend ones are the first backend deploys in a while and the backend has no test gate at all**, so nothing but CI-on-push stood between them and production. Plus three older: `8d113ba` (author listing aggregation, a **public read path** — confirm the real 3,300-author collection returns the same letters and counts as before), `8283b88` (notification fan-out + index) and `d3adb85` (notifications UI).
- **This week's SEO work, six commits, all frontend** — `6218d7a` (503 on backend outage), `4698dd0` + `37c0135` (genre introductions, now 132), `9f756cd` (famous poems noindex + sitemap), `24885e5` (introduction collapsed behind "Read more"). Spot-checked on the deployed URL as each went out; what is NOT verified is how the introductions read to a person on a phone, and whether the "Read more" control is obviously tappable at 375px.

- **The header on a real phone, portrait.** Fixed 2026-08-11 and measured from 320px up, but confirm on the device: all five controls visible (menu, wordmark, bell, avatar, logout), nothing clipped, and the Categories menu still opens.
- **The notifications panel on a real phone.** Fixed 2026-08-11 and measured across 13 widths in a harness built from the real compiled SCSS, but a harness is not a phone: confirm the panel opens fully on screen, that nothing below it scrolls sideways, and that it still sits under the bell on a tablet.
- **Publishing a poem still works.** `c42e4ca` added validation to the create route and now stores `title`/`poem` trimmed — the most user-facing write on the site. Publish one real poem and confirm it saves, appears in the list, and its slug resolves.
...
- 🤝 **(MEDIUM) `GET /api/v1/comments?since=1970-01-01` dumps every comment on the site** — `backend/src/controllers/comments.js:209`. Unauthenticated, unpaginated, populated with author name/slug/picture. It exists for the simulation scripts. Separately, the by-target branch has no draft check, so comments on a withdrawn poem stay publicly readable — the exact case `/mine` and `/received` go out of their way to filter. **Product call**: do the scripts still need `since=`, or can it move behind `SIMULATION_INTERNAL_SECRET`?
- 🤖 **(LOW) Audit smalls, one commit.**
  - `poems.js:485` — admin `PATCH /api/v1/poems` runs `updateMany({}, { $set: req.body })` over all 16k poems with no allowlist and no dry-run. `{"status":"draft"}` unpublishes the site in one request, and every other bulk operation in this repo is dry-run by default.
  - `poems.js:263` — the author-not-found early return hardcodes `page: 1, limit: 10` whatever was asked for.
...
- 👤 **Read the genre introductions — they are the site's voice now, not a draft.** All 132 are live (2026-08-18, `frontend/src/data/genreIntros.ts`). They were written by an LLM at your instruction, and the whole reason a genre page can rank is that its prose is TRUE and yours; a confident sentence about a poem that does not say what is claimed is the exact "written for search engines" signal the work exists to escape.

  **Where the risk is concentrated**, so this is a couple of hours and not a week: the thin shelves, where a single confident claim carries a whole entry — Kindness, Trust, Apology, Brother, Sister, Justice, Funeral, God, Bullying, Long Distance. The big ones make more claims but each is more standard.

  Several are deliberately corrective and are the ones most worth checking: that "The Road Not Taken" says the two roads were worn about the same; that Dickinson never claims the bird with feathers is right; that "In Flanders Fields" asks for the quarrel to be taken up rather than for peace; that Frost's "home is the place where..." is said by one character and disputed by another.

- 🤝 **Eleven categories still have no introduction** — the same eleven that hold no poems (see P4). Deliberate: `/[genre]` marks an empty genre `noindex`, so prose there is an essay over nothing. They are a one-line addition each the moment a poem lands, and `genreIntros.test.tsx` asserts the count, so the gap cannot be forgotten silently.

- 🤝 **The AI poems are still indexed — decide whether they stay** (raised 2026-08-17, `docs/SEO_AUDIT.md` P1). With the famous poems now `noindex`, the indexable poem corpus is **435, of which 416 are AI-generated** — the same scaled-content shape at 1/36th the scale. The AI disclosure the footer and per-poem badges make to READERS has no equivalent in the index. `noindex`ing them is cheap (the sitemap already splits community out; the condition is `origin === 'ai'` next to the famous one) and would leave an indexable corpus of 19 poems plus the taxonomy — small, honest, and able to rank. Not urgent, and genuinely a product call.

- 🤝 **The scraped corpus includes living, in-copyright poets** (raised 2026-08-17). `seed-poems.js` records the source as `seed@poetryfoundation.org`, and a sample of what it pulled includes Joseph O. Legaspi, Dani Couture, Susan Elizabeth Howe and Andrew Shields — full texts of contemporary poets. **This is legal exposure, not an SEO question**, and `noindex` does not resolve it: the pages are still publicly served. The narrow fix, if you want one, is to remove the in-copyright poems and keep the public-domain canon (Dickinson, Donne, Whitman, Housman, Keats), which would preserve the collections that make the genre pages worth browsing. Scoping how many are affected is a script, not a decision.

- 🤖 **Poem pages are near dead-ends — add "More by this author" and "More in this genre"** (`docs/SEO_AUDIT.md` P3). Measured on the live site: a poem page carries exactly **one** outbound poem link (the Next-poem card), so 16,087 poems form a single linked list and reaching the *n*th costs *n* hops. The author's other poems are already loaded on the page; the genre module is one extra query. It is a reader improvement first and worth doing regardless of the noindex decisions above.

- 🤖 **Pagination strands the middle of large genres** (`docs/SEO_AUDIT.md` P4). The nav on `/love?page=63` links first, last and ±2 — so page 63 of 125 is ~30 hops from either end. Widen the window, or add paginated list URLs to `pages.xml`. Secondary: those poems are reachable through author pages and the sitemap, so this is signal flow rather than discovery.

- 🤖 **Sitemap and heading smalls, one commit** (`docs/SEO_AUDIT.md` P5, ~an hour):
  - `authors.xml` emits **no `<lastmod>` on any of its 3,364 URLs** — the other sections all have it, and Google uses it for recrawl scheduling.
  - Drop `<changefreq>` and `<priority>` from all sections. Google has ignored both for years; it is 19,587 × 2 lines of noise.
  - The 26 letter pages all have `<h1>Authors</h1>` while their `<title>` correctly says "Poets starting with B". The H1 should carry the letter.
  - `/love?page=2` has the same `<h1>` as page 1, though the title does distinguish them.

- 👤 **Validate the soft-404 fix in Search Console.** Fixed in `6218d7a` (backend outage answered 200-with-nothing; now 503 + `Retry-After`). GSC → Indexación → Páginas → Soft 404 → **Validar corrección**, then watch the 1,025 revalidate. If they come back as 503 rather than 200s, that is the fix working — and it points at the edge-caching item below.

- 🤖 **One author slug is a typo in the database**: `wisawa-szymborska` (missing the `ł`/`l`). Found while verifying the curated links. Harmless in isolation, but it is a live URL and a hint that the scrape's transliteration dropped other diacritics — worth one pass over the author slugs for the same pattern.

- 🤖 **Curated author links need re-checking after any author migration.** `node frontend/scripts/check-author-slugs.mjs` verifies all 837 against production (read-only) — 528 `startHere` links on the genre introductions plus 309 `readNext` links on the author ones, 221 distinct poets; `--refresh` rewrites the committed snapshot the unit tests assert against. The test catches a slug that was wrong when written; only the script catches one retired later.
...
- **The dashboard showed five scrollbars at once** (2026-08-21) — page, left rail, Categories, Authors, right rail, for two columns of about twenty links. `.accordion__list`'s 350px cap was written unscoped in `Header.scss`, so the sidebar nested a scroller inside `.dashboard__accordion`, which is already one. Scoped to `.header__dropdown`, where it is genuinely needed. Measured in headless Chrome at 1728x950: three nested scrollers before, one after (the right rail, deliberate). Pinned by `nestedScrollers.test.ts`, which compiles the real SCSS — jsdom computes no layout, so RTL cannot see this.
- **Editorial introductions on every author page with 20+ poems** (2026-08-21) — extends the 30+ tier shipped 2026-08-18 by 63 poets, for 103 of 3,364. Same rules and the same two guards: no AI persona gets one (five now clear the threshold), and the component renders only for `authorType === 'famous'`. Below 20 poems the remainder average under five each, where an essay outweighs the collection it introduces. Adds 189 author-to-author links.
