...
- 🤖 **The sitemap carries no `<lastmod>`** (found 2026-08-04). 19,598 URLs, not one timestamp. Google therefore has no signal that a page changed, which is why RESUBMITTING the sitemap after the SSR fix would have achieved nothing — a resubmit prompts discovery of new URLs, and none of these are new. With this many URLs the crawl budget is spread thin, so `lastmod` is what makes any future change recrawl promptly. Poem pages can use the poem's own date; listing pages the newest poem they contain. Pair it with the sitemap-index split below, since both touch the same generator. **In the meantime**, the way to get a hub re-fetched is Search Console → Request indexing on the handful that matter (homepage, top genres) — not a resubmit.

- 🤖 **Speed up the sitemap** — `pages/sitemap.xml.ts` pages the poems API 100 at a time (~160 sequential calls for 16k poems → ~25 s cold generation, risking the serverless timeout). It's already CDN-cached (`s-maxage=86400`), so this only bites on a cold cache. To fix, raise the poems endpoint's `limit` cap (currently `Math.min(limit, 100)` in `poems.js`) so the sitemap can fetch bigger pages, or split into a sitemap index + per-section child sitemaps. Backend change, deferred.
