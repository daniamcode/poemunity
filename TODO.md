...
- 🤝 **Every page is uncacheable — decide whether to make public pages edge-cacheable.** (From the SEO audit, 2026-08-07.) Every SSR route returns `cache-control: private, no-cache, no-store, max-age=0, must-revalidate`, which is Next's default when `getServerSideProps` does not set one. Nothing caches at Vercel's edge, so all 16,087 poem pages cost a full render plus a backend round-trip on **every** Googlebot fetch — and crawl rate is throttled by how expensive a site is to crawl. Measured TTFB is fine (0.3-0.55s); the issue is that it is paid every single time.
  ...
  **Deliberately parked until the Search Console per-sitemap numbers arrive** (the item above). The tradeoff being bought here is a signed-out header flash for every logged-in reader, and it is only worth paying to make ~16k pages cheap to crawl — but if those numbers say the 15,652 famous poems are unindexable duplicates and they get `noindex`ed, 97% of that crawl cost disappears on its own and this stops being worth the flash. Decide the sitemap question first; this one falls out of it either way.

  Its small sibling is **done** (2026-08-10): the sitemap routes' `stale-while-revalidate` had no value, so normalisation dropped it and only `public` survived. Now `SITEMAP_CACHE_CONTROL` in `src/lib/sitemap.ts`, shared by both routes. 👤 **Still needs confirming against the live response headers** — `curl -sI https://poemunity.com/sitemaps/poems-famous.xml` on two consecutive requests should show `x-vercel-cache: HIT` and a rising `age:` on the second. A test can pin the string; only the deployed response can prove the CDN honoured it.
...
- 🤖 **`/login` and `/register` have no `<h1>`.** Accessibility rather than SEO — both are `noindex,nofollow` — and the last leftover from the SEO-smalls item shipped 2026-08-10, which covered the other five bullets.
