...
### Index drift in Atlas (one pass, both items together)

- 🤖 **Drop the redundant `recipient_1` index.** The schema change shipped in `8283b88`, so nothing recreates it, but `autoIndex` only ever CREATES and the index built in Atlas is still there, costing a write on every notification insert and collapse. `check-index-drift.js` reports it as orphaned; `node backend/scripts/drop-redundant-notification-index.js` (dry-run by default, `--apply` to act) drops it. `mongodump` first.
- 🤖 **Confirm the `Comment` index actually built.** `{ authorId: 1, createdAt: -1, _id: -1 }` was added for the My comments tab and builds itself on deploy — but that has never been checked, and without it the query scans every comment. `Comment` is already in the `MODELS` list.

  Both are answered by one read-only run of `node backend/scripts/check-index-drift.js`, which also confirms the 3 follow + 2 notification indexes built at all — unchecked since either feature shipped. See Standing rules for why `autoCreate` must be off too.
