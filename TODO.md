...
- 🤖 **Curated `startHere` links need re-checking after any author migration.** `node frontend/scripts/check-author-slugs.mjs` verifies all 528 against production (read-only); `--refresh` rewrites the committed snapshot the unit test asserts against. The test catches a slug that was wrong when written; only the script catches one retired later.
