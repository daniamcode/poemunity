...
Prompted by "is `/home` a duplicate of `/`?" — **it is not**: `Home` is a real entry in `CATEGORIES`, so `/home` is the genre page (63 poems), with its own title, its own canonical and a sitemap entry. Nothing to fix there. The investigation did surface three real things:
...
- 👤 **The preference that would not persist.** During the 2026-08-04 session a notification preference read as ON in the UI while comment notifications were not arriving; it resolved on its own and was never root-caused. The toggle at the time gave NO feedback either way — it sent the PATCH and waited, so a failed save looked exactly like a successful one. The current version reverts the box when the save fails, so a recurrence is visible rather than silent. If it happens again, capture the network response before anything else.
