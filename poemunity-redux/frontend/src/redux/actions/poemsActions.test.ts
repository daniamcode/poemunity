...
import {
    createPoemAction,
    getAllPoemsAction, updateAllPoemsCacheAfterLikePoemAction, updatePoemsListCacheAfterLikePoemAction, updateRankingCacheAfterLikePoemAction
} from "./poemsActions";
...
describe('createPoemAction', () => {
    ...
    test('Should dispatch error when axios throws a generic error', async() => {
        (axios.create as jest.Mock).mockReturnThis();
        (axios.post as jest.Mock).mockReturnValueOnce(Promise.reject({response: 'some error'}));
    ...
    test('Should dispatch response when axios returns data correctly', async() => {
        // this is beacuse we use Axios.create
        (axios.create as jest.Mock).mockReturnThis();
        (axios.post as jest.Mock).mockReturnValueOnce(Promise.resolve({data:'poem1'}));
